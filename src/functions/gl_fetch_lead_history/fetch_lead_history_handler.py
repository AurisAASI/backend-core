...
def _parse_body(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(comm_ids, list) or not all(
        isinstance(x, str) and x.strip() for x in comm_ids
    ):
...
def _get_communication_by_id(
    communication_id: str, db: DatabaseHandler
...
def fetch_lead_history(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        communication_db_handler = DatabaseHandler(
        ...
        lead_ids = {c.get('leadID'): None for c in communications}
        # Remove empty keys
        ...
        logger.info(
            f'Returning {len(entries)} history entries after enrichment and sorting'
        )
        return response(
            status_code=HTTPStatus.OK,
